...
fn test_vm_performance() {
    ...
    let opts = JS::new(1, Atom::from("test vm"), 0, 0, 0, Arc::new(NativeObjsAuth::new(None, None)), None);
    assert!(opts.is_some());
    let js = opts.unwrap();
    ...
    for vm_id in 0..10000 {
        JS::new(vm_id, Atom::from("test vm"), 0, 0, 0, Arc::new(NativeObjsAuth::new(None, None)), None).unwrap().load(codes.as_slice());
    }
